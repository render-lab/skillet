...
import { HIGH_THRESHOLD, MID_THRESHOLD } from "../utils/rate.js";

/**
 * Writes a single index.html dashboard that loads JSON result files client-side.
 * Call this after every run to keep the dashboard up-to-date.
 */
export function writeDashboard(resultsDir: string) {
	const outputPath = path.join(resultsDir, "index.html");
	const html = DASHBOARD_TEMPLATE.replace("__HIGH_THRESHOLD__", String(HIGH_THRESHOLD)).replace(
		"__MID_THRESHOLD__",
		String(MID_THRESHOLD),
	);
...
<title>skillet eval dashboard</title>
...
async function init() {
  try {
    ...
    $("#app").innerHTML = '<div class="error-banner">Failed to load results. Make sure you\\'re using <code>skillet eval serve</code>.</div>';
  }
}

function renderList() {
  if (!runs.length) {
    $("#app").innerHTML = '<h1>skillet eval</h1><p class="subtitle">No results yet</p><div class="empty">Run <code>skillet eval run</code> to generate results.</div>';
    return;
  }
