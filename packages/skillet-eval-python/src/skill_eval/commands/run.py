...
async def run_run(opts: RunOpts) -> None:
    ...
    if not Path(paths["evals_file"]).exists():
        ...
        console.print(
            f"\nRun [bold]skillet-eval generate {opts.skill}[/bold] to auto-generate evals from SKILL.md,\n"
...
def _print_run_header(
    ...
    console = Console()
    console.print(f"\n  [bold]skillet-eval v{__version__}[/bold]\n")
...
def _write_outputs(
    ...
    console.print(
        f"\n  [dim]Run [bold]skillet-eval serve {opts.skill}[/bold] to view results in the browser[/dim]\n"
    )
