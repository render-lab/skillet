...
export async function runRun(opts: RunOpts) {
	...
	if (!fs.existsSync(paths.evalsFile)) {
		...
		console.error(
			`\nRun ${pc.bold(`skill-eval generate ${opts.skill}`)} to auto-generate evals from SKILL.md,\nor create evals.json manually.\n`,
		);
		process.exit(1);
	}
...
function writeOutputs(
	...
	console.log(
		`\n  ${pc.dim(`Run ${pc.bold(`skill-eval serve ${opts.skill}`)} to view results in the browser`)}\n`,
	);
