...
app = typer.Typer(
    name="skill-eval",
    help="Provider-agnostic skill evaluation tool",
...
@app.command()
def init() -> None:
    """Scaffold a skill-eval.yaml config file interactively."""
    _load_env_for_skill()
...
def generate(
    ...
    config: str | None = typer.Option(None, "--config", help="Path to skill-eval.yaml"),
) -> None:
...
def _version_callback(value: bool) -> None:
    if value:
        typer.echo(f"skill-eval v{__version__}")
        raise typer.Exit()
